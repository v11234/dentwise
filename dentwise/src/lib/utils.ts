...
export function formatPriceWithFCFA(price: string, rate = 600) {
  const match = price.match(/\$([\d.]+)/);
  if (!match) return `${price} - ~${(20).toLocaleString()} FCFA`;
  const usd = Number(match[1]);
  const cfa = Math.round(usd * rate);
  return `${price} (≈ ${cfa.toLocaleString()} FCFA)`;
}

export function parseUsdFromPrice(price: string | undefined) {
  if (!price) return 0;
  const match = price.match(/\$([\d.]+)/);
  if (!match) return 0;
