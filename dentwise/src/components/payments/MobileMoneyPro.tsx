...
const PLAN_FEATURES: Record<Props["plan"], string[]> = {
  pro: ["50 AI dental consultations", "Priority appointment booking", "Email support"],
  premium: [
...
export default function UpgradeCard({ plan, price }: Props) {
  ...
  return (
    <article className="relative rounded-3xl border border-border/70 bg-card/90 p-6 shadow-sm transition hover:shadow-md">
      <div className="mb-6 flex items-start justify-between gap-3">
        <div className="space-y-2">
          <Badge variant="secondary" className="capitalize">
            {plan} plan
          </Badge>
          <h3 className="text-2xl font-semibold tracking-tight">{price} FCFA</h3>
          <p className="text-sm text-muted-foreground">
            per month, choose mobile money or bank transfer
          </p>
        </div>
      ...
      <div className="space-y-3">
        {method === "mobile_money" ? (
          <>
            <label htmlFor={`${plan}-phone`} className="text-sm font-medium">
              Mobile money number
            </label>
            <Input
              id={`${plan}-phone`}
          ...
          <>
            <label htmlFor={`${plan}-bank-name`} className="text-sm font-medium">
              Your bank name
            </label>
            <Input
              id={`${plan}-bank-name`}
            ...
            <label htmlFor={`${plan}-account-number`} className="text-sm font-medium">
              Your bank account number
            </label>
            <Input
              id={`${plan}-account-number`}
