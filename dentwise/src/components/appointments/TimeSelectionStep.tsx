...
import { APPOINTMENT_TYPES, formatPriceWithFCFA, getAvailableTimeSlots, getNext5Days } from "@/lib/utils";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-2xl font-semibold">{t("appointments.timeSelection.title")}</h2>
      </div>

      <div className="grid lg:grid-cols-2 gap-8">
        {/* appointment type selection */}
        <div className="space-y-4">
          <h3 className="text-lg font-medium">{t("appointments.timeSelection.appointmentType")}</h3>
              ...
              >
                <CardContent className="p-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-primary">{formatPriceWithFCFA(type.price)}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        {/* date & time selection */}
        <div className="space-y-4">
          <h3 className="text-lg font-medium">{t("appointments.timeSelection.availableDates")}</h3>
          ...
          {selectedDate && (
            <div className="space-y-3">
              <h4 className="font-medium">{t("appointments.timeSelection.availableTimes")}</h4>
                ...
                {availableTimeSlots.map((time) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {time}
                      {isBooked && ` (${t("appointments.timeSelection.booked")})`}
                    </Button>
                  );
                })}
              </div>
      ...
      {selectedType && selectedDate && selectedTime && (
        <div className="flex justify-end">
          <Button onClick={onContinue}>{t("appointments.timeSelection.review")}</Button>
        </div>
      )}
    </div>
  );
